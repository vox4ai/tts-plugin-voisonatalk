...
class VoiSonaTalkConnector(TTSConnector):
    ...
    SUPPORTED_PARAMS = ["alp", "huskiness", "intonation", "pitch", "speed", "volume", "style_weights"]
    ...
    async def synthesize(self, req: TTSRequest) -> TTSResponse:
        ...
        try:
            ...
            while (asyncio.get_event_loop().time() - start_time) < self.max_wait:
                async with session.get(f"{self.api_url}/speech-syntheses/{uuid}") as resp:
                    if resp.status != 200:
                        break
                    ...
                    if state == "succeeded":
                        ...
                        return TTSResponse.fail("VoiSona Talk reported synthesis failure")
